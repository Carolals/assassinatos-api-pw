...
const getVitimas = async (request, response) => {
    await getVitimasDB()
          ...
          .catch(err => response.status(400).json({
            status : 'error',
            message : 'Erro ao consultar os cadastros das vitimas: ' + err
          }))
}
...
const addVitima = async (request, response) => {
    await addVitimaDB(request.body)
          .then(data => response.status(200).json({
                status : "success", message : "cadastro de vitima criado",
                objeto : data
          }))
          .catch(err => response.status(400).json({
            status : 'error',
...
const updateVitima = async (request, response) => {
    await updateVitimaDB(request.body)
          .then(data => response.status(200).json({
                status : "success", message : "cadastro da vitima alterado",
                objeto : data
          }))
          .catch(err => response.status(400).json({
            status : 'error',
